...
export const multerOptions = (dest: string) => ({
  ...
  fileFilter: (req: any, file: any, callback: any) => {
    ...
    if (dest === 'chat') {
      if (!file.originalname.match(/\.(jpg|jpeg|png|gif|webp|mp3|wav|m4a|ogg|webm)$/)) {
        return callback(new BadRequestException('Only images and audio files are allowed for chat!'), false);
      }
    } else {
